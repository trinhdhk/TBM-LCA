...
tibble::tribble(
  ...
  'CSF Total While cell Count'     , '^^[Risk of TBM increases if White Cell count is in a bounded range], weak' , '+, weak'    ,
...
By protocol, most patients with very high chance of or evidently diagnosed with different diseases were not tested with TBM confirmation assays,wWe assumed that patients who had no TBM confirmation tests are all negative, because these tests are assumed to have high specificity [@nhu2013, @heemskerk2018]. Missing predictors were handled case by case basing on their expected missing mechanisms (table \@ref(tab:missing-handling)). In case they were expected to be Missing At Random (MAR), we performed a model-based imputation. We detailed of our imputation strategy in append \ref(appendix-imputation).

For TBM confirmatory tests sensitivity and specificity, we chose priors so that they could cover past estimations from different studies involving Vietnamese cohort [@thwaites2004; @nhu2013; @heemskerk2018] (figure \@ref(fig:mv-priors)). Positive Rate (FPR = 1-Specificity) and weakly informative priors for True Positive Rate (TPC = Sensitivity) on the logit scale, given the discrepancies between different research. 

All model performances were estimated and compared by (1) expected log point-wise predictive density (elpd, @vehtari2016), (2) calibration [@rms, @VanCalster2019], and (3) Receiver Operating Characteristic (ROC) curve and corresponding Area Under the Curve (AUC). All metrics were estimated using a pooled dataset held out from 5 repetitions of 20-fold cross validation. Results from best performed model were report. Validity of our assumptions with respects to imputation and prior choices were separately checked by sensitivity analyses. Pair-wise residual correlation were plotted to validate the local indepence assumption, after correction for local effects [@qu1996; @schumacher2016]. For demonstration, we visualised class-wise predicted probability density plots. This depicts how separable predicted values are between two classes. We also used the diagnosis at discharge or as a pseudo-gold standard to visualise the correlation of our model-based prediction and delayed clinical diagnosis. 

To explore hidden effects, we added quadratic terms for all five CSF bio-markers and RGCS; CSF volume was additionally included to capture its potential impact on test sensitivity.

We lastly developed a simplified model recovering the information without laboratorial features. To achieve this, we transferred the analytic forms of posterior distributions from the best performant model, on the logit scale to a different linear system where the number of predictors were reduced.
...
All data preparation, cleaning, and processing were performed on statistical package R, version 4.1.0 [@rcoreteam]. The model was developed on the probabilistic language Stan via the interface Rstan , version 2.27 [@stan]. Plotting was done using package bayesplot [@bayesplot], classifierplots [@classifierplots], and ggvenn [@ggvenn]. Some other packages used include: R6 [@r6] and rms [@rms].

# Results

## Clinical characteristics

From 29th August 2017 to 22nd January 2021, there were 689 patients enrolled in the main study, amongst them `r nrow(data_19EI)` were included in the analysis. Characteristics of analysed population are shown in table \@ref(tab:test-bl-tbl); ZN Smear returned positive for `r sum(data_19EI$csf_smear, na.rm=TRUE)` individuals, MGIT Culturing for `r sum(data_19EI$csf_mgit, na.rm=TRUE)` and Xpert for `r sum(data_19EI$csf_xpert, na.rm=TRUE)` (figure \@ref(fig:venn-test)).
...
### Confimatory test positive rates



### Risk factors for TBM and scoring system
...
bl_tbl |> as_flex_table() |>
  flextable::set_caption('Patient Characteristics') |>
  flextable::width(width = 1.25) |>
