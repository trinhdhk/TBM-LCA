...
Basic desgins for all models is shown in figure \@ref(fig:skeleton-model). We considered different model designs in which we incrementally added up more flexibility and lifted more constraints [@schumacher2016] (table \@ref(tab:model-archs)). As supported by background knowledge[@marais2010], only CSF White Cell count was modeled via a quadratic effect.

```{r skeleton-model, fig.show='hold', out.width="100%", fig.align="center", fig.cap="Model basic design. Bacillary burden is only included in model 2. The test positive rates are for demonstration only.", fig.id='skeleton-model'}
...
We also used the diagnosis at discharge time at a pseudo-gold standard to visualise the calibration and AUC for the predicted TBM prevalence (@ref(fig:theta-metrics)). The AUC for TBM prediction were `r classifierplots:::calculate_auc(test.y = !data_19EI$other_dis_dx, pred.prob = m3$p$theta$mean) |> round(2)` and calibration plots show quite small over-estimation at the centre. 
...
theta_rocs <- 
  my_fn$my_roc_plot(obs = !data_19EI$other_dis_dx, pred = m3$p$theta$mean) + ggtitle('Smear') + theme_bw() + plot_theme
theta_calib <- td.misc::binary_calibration(pred=m3$p$theta$mean, obs=!data_19EI$other_dis_dx, span=1) + plot_theme
...
Through the implication of LCA, our model succesfully estimated the positive rates for corresponding confirmation procedures. We founds out that GeneXpert, in combination with Xpert Ultra, was not superior to other confirmatory tests, at least for TBM diagnosis, contrary to its popularity and benefit for pulmonary TB <citation>. Compared to estimations the most recent one [@donovan2020], where the authors used one subset of data with 305 patients, the estimated sensitivity were marginally lower for all three assays, with ZN Smear stood at 67% (compared with 71.3%), MGIT culture 31% (47.9%), and Xpert was 13% (39.6%). However, the latter were against standard uniform definition which is in favor of the confirmatory tests themselves, whereas ours were calculated against the actual TBM status, regardless of the test. This might also suggest a small misdiagnosis of TBM when using the current approach, particularly for those with negative test results <may be we can calculate prob TBM in case all test negative to demonstrate this>. This underperfomance was again shown in the calibration plot against discharge diagnosis, with a small dip of observed risk at the around 50%. In fact, we all agree that none amongst the three confirmation indicators are reliable to be used alone as gold standard, and a true gold standard is urgently needed. They, however, all performs well in TBM negative class, with mostly no false positive. These estimations reflect well the biological mechanisms of the tests.

Despite being design for research, due to the categorical nature, current TBM definition studies usually suffer difficulty in which group of patients should be included in the cohort. A study of only definite TBM might favour severe patients while a lower group might have more false positive mixed with them. Choosing a cut-off usually comes with large trade-off, as a large group of patient would be excluded or included. By providing a probability, our model can benefit both clinicians and researchers with more clear insight, and more flexible choices of cut-off. In prognosis research, this probability, together with the ordinal quanitification of bacillary burden, are correction parameters for patients' responses to TBM treatment regimen, which is usually quite specific.

Another finding of our study is the correction of current known diagnostic criteria. In comparison to widely-used consensual scores, some risk factors considered important when diagnose TBM (TB-suggested symptoms, cranial nerve palsy) have minimal contribution in our model. HIV, albeit not mentioned in the original score, turns out to be very important. Laboratorial parameters in general follow the same impact as background belief. Basing on our posterior estimations, we proposed a formula for TBM probability <cross-ref> and a corrected scoring system in table <to-be-added>. A Shiny app was built as a convenient tool for research purpose. In clinical settings, we also provide a simplified scoring system, based on our simplification model. This system only needs demographic and clinical inputs, and are usually easier to diagnose, particularly as early as admission time.
