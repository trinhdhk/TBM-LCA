...
HIV were tested for `r nrow(data_dirty) - na(data_dirty$hiv_stat)` (`r round((nrow(data_dirty) - na(data_dirty$hiv_stat))/nrow(data_dirty)*100, 2)`%) enroled individuals and `r n_test_suspected_hiv` (`r (n_test_suspected_hiv/nrow(suspected_hiv)*100) |> round(2)`%). `r na(data_dirty$clin_symptoms)` patients did not provide information for at least one in three TB-suggested symptoms. Amongst `r na(data_dirty$clin_gcs)` individuals with incomplete GCS, GCS Vocal was missing for `r na(data_dirty$GCSV)`.  As HIV was only done for suspected patients who admitted with HIV-induced condition or involved in high-risk activities, we assumes the HIV rate for the untested group to be similiar to that of the general population. Missing GCSV due to intubation is a common issue and the practice of using a model-based imputation approach is supported[@Teasdale2014], as all the Glasgow Coma Scale components are usually highly correlated.
...
Another finding of our study is the statistical-based estimation of individual TBM probability and contribution of current known diagnostic criteria. From our data, we witnessed that some used predictors like TB-suggested symptoms and cranial nerve palsy have minimal impact on the chance of TBM. This is contrary to previous consensual definition[@marais2010] where the former had a score of 2 and the latter of 1. On the other hand, HIV, albeit not mentioned in the original score, turns out to be very important, with an estimated coefficient of `r a_plot$data['a[1]','m']`. The case of HIV has been well-known, to remedy this, usually, doctors just considered two sub-population differently, added a implicit bonus point for HIV positive patients.  TB-suggested symptoms, in spite of its name, are very non-specific and are prone to noise, while cranial nerve palsy might also appears in several neurological diseases. Laboratorial parameters in general follow the same trend as background knowledge.
