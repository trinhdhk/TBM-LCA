...
```
HIV were tested for `r nrow(data_dirty) - na(data_dirty$hiv_stat)` (`r (nrow(data_dirty) - na(data_dirty$hiv_stat))/nrow(data_dirty)*100`%) enroled individuals and `r n_test_suspected_hiv` (`r n_test_suspected_hiv/nrow(suspected_hiv)*100`%). `r na(data_dirty$clin_symptoms)` patients did not provide information for at least one in three TB-suggested symptoms. Amongst `r na(data_dirty$clin_gcs)` individuals with incomplete GCS, GCS Vocal was missing for `r na(data_dirty$GCSV)`. 
...
```{r best-metrics, out.width='100%', fig.dim=c(8,6), fig.align='center', fig.asp=1, fig.cap = 'Performance plots for selected model', echo=FALSE, message=FALSE}
...
wrap_plots(rocs, calib, ncol=1)

```

The selected model provides good discrimination between two TBM status.
