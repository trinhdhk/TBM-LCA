...
#   # browser()
#   theta_cs = cs_a0[i,] + cs_a[i,1]%.*%hiv2$pred[id,1] + cs_a[i,2]%.*%id2$pred[i,id] + cs_a[i,3]%.*%obs[id] + cs_a[i,4]%.*%test[id]
#   
...
#                                 sapply(id, function(j) {
#                                   gcs_mu = gcs_a0[i] + gcs_a[i,,1]%.*%X[i,j,1] + gcs_a[i,,2]%.*%obs[j] + gcs_a[i,,3]%.*%test[j]
#                                   GCS = sapply(id,
#                                                function(...) {
