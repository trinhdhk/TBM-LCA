  ...
  sapply(seq_len(dim(hiv_a0)[1]),
         function(i){
           sapply(id, function(j){
             pnorm(hiv_a0[i] + hiv_a[i,1]*obs[j] + hiv_a[i,2]*test[j])
           })
...
obs_Xd[Td[,7]==0,1] = 1
hiv = fill_x(Xd[Td[,7]==1,1], obs_Xd[Td[,7]==1,1], hiv_pred, full=F)
# hiv2 = fill_x_holdout(Xd[Td[,7]==1,1], obs_Xd[Td[,7]==1,1], hiv_pred, lapply(holdout, \(x) x[Td[,7]==1]), full=T)
hiv3 = fill_x(Xd[,1], obs_Xd[,1], hiv_pred, full=T)
...
id2 = fill_x(Xc[,1], obs_Xc[,1], id_pred, full=TRUE)
...
clin_symptoms2 = fill_x(Xd[,2], obs_Xd[,2], cs_pred_binary, full=T)
clin_symptoms3 = fill_x(Xd[,2], obs_Xd[,2], cs_pred_prob, full=T)
...
motor_palsy2 = fill_x(Xd[,3], obs_Xd[,3], mp_pred_binary, full=T)
motor_palsy3 = fill_x(Xd[,3], obs_Xd[,3], mp_pred_prob, full=T)
...
csf = fill_x_multi(Xc[,2:7], obs_Xc[,2:7],csf_pred, full=T)
...
gcs_pred = function(model, id){
  ...
  # future::plan(future::multisession, workers=0)
  ...
  sapply(seq_len(dim(L_omega_gcs)[1]), 
                              FUN = function(i){
                                ...
                                sapply(id, function(j) {
                                  ...
                                  (round(3*GCS[,1] + 5*GCS[,2] + 4*GCS[,3]))/3 - 1;
                                })
                              }) |> t()
}
# gcs = fill_x_holdout(Xc[,8], obs_Xc[,8], gcs_pred, holdout)
gcs2 = fill_x(Xc[,8], obs_Xc[,8], gcs_pred, full=T)
#gcs2$pred = 12 - 3*gcs2$pred
...
hiv_miss = prob_missingness(obs_Xd[Td[,7]==1,1], 
                            lapply(seq_len(dim(gcs2$pred)[1]),
                                   function(i) 
                                     cbind(clin_symptoms2$pred[i,Td[,7]==1], motor_palsy2$pred[i,Td[,7]==1], id2$pred[i,Td[,7]==1],
                                           gcs2$pred[i,Td[,7]==1],
...
#hiv_pred_smooth[, obs_Xd[Td[,7]==1,1]==1] = hiv_all
#hiv_pred_smooth[, obs_Xd[Td[,7]==1,1]==1] = sapply(seq_len(dim(hiv_miss)[1]), 
 #                        function(i) loess(Xd[obs_Xd[,1]==1 & Td[,7]==1,1]~hiv_miss[i, obs_Xd[Td[,7]==1,1]==1],  control=loess.control(surface="direct")) |> predict()) |> t()

hiv_plot = list()

# hiv_plot$plot0 = 
#   ggplot() +
#   geom_density(aes(x=hiv_pred_smooth[, obs_Xd[Td[,7]==1,1]==0]), color=color$pred, adjust=4) +
#   geom_vline(aes(xintercept=mean(hiv$obs)), color=color$obs) +
#   theme_bw() +
...
hiv_plot$plot1 = 
  ...
  # geom_hex(aes(y=hiv2$pred[1:100,Td[,7]==1], x=hiv_miss[1:100,], fill = rep(as.logical(obs_Xd[Td[,7]==1,1]),each=100), alpha=..ncount..)) + #fill = rep(as.logical(obs_Xd[Td[,7]==1,1]), each=2)
   geom_hex(aes(y=hiv2$pred[1:200,Td[,7]==1], x=hiv_miss[1:200,], fill = rep(as.logical(obs_Xd[Td[,7]==1,1]),each=200), alpha=..ncount..)) + #fill = rep(as.logical(obs_Xd[Td[,7]==1,1]), each=2)
  ggfx::with_outer_glow(colour = '#ffffff', sigma=1, expand=3, x = geom_smooth(aes(y=hiv2$pred[1:200,Td[,7]==1], x=hiv_miss[1:200,], color = rep(as.logical(obs_Xd[Td[,7]==1,1]),each=200), fill = rep(as.logical(obs_Xd[Td[,7]==1,1]), each=200)), alpha=.5, method='loess', se=F)) + 
...
hiv_plot$plot2 = 
  ggplot()+
  geom_density(aes(x=resid_missingess(hiv2$pred[1:4000,Td[,7]==1], hiv_miss[1:4000,]), fill=rep(as.logical(obs_Xd[Td[,7]==1,1]),each=4000)), color='transparent', alpha=.5, adjust=.5) +
  scale_discrete_manual(c('color', 'fill'), name = '', breaks = c(FALSE, TRUE), labels=c('Imputed', 'Observed'), values = unlist(color) |> setNames(c('FALSE', 'TRUE')))+
  xlab('Residual') +
  ylab('Density') +
  theme_bw()
...
hiv_plot$plot3 = 
  ggplot()+
  geom_hex(aes(y=resid_missingess(hiv2$pred[1:100,Td[,7]==1], hiv_miss[1:100,]), x = hiv_miss[1:100,], fill=rep(as.logical(obs_Xd[Td[,7]==1,1]),each=100), alpha = ..count..)) + 
  ggfx::with_outer_glow(colour = '#ffffff', sigma=1, expand=3, x = geom_smooth((aes(y=resid_missingess(hiv2$pred[1:100,Td[,7]==1], hiv_miss[1:100,]), x = hiv_miss[1:100,], color=rep(as.logical(obs_Xd[Td[,7]==1,1]),each=100))), method='loess', se=FALSE)) +
  scale_x_continuous(trans='logit', breaks = c(.25, .75, .95, .99, .999), limits=c(0.2, .9999)) +
  scale_discrete_manual(c('color', 'fill'), name = '', breaks = c(FALSE, TRUE), labels=c('Imputed', 'Observed'), values = unlist(color) |> setNames(c('FALSE', 'TRUE')))+
  scale_alpha_continuous(range=c(0.2,.7), trans='log10') + guides(alpha='none')+
  xlab('Pr (Response)') +
  ylab('Residual') +
  theme_bw()
...
cs_plot = list()
#cs_pred_smooth = clin_symptoms2$pred
#cs_pred_smooth[,obs_Xd[,2]==1] = sapply(seq_len(dim(cs_miss)[1]), 
                         #function(i) loess(Xd[obs_Xd[,2]==1, 2]~cs_miss[i, obs_Xd[,2]==1]) |> predict()) |> t()


cs_plot$plot1 = 
  ggplot() + 
  geom_hex(aes(y=clin_symptoms2$pred[1:100,], x=cs_miss[1:100,], fill = rep(as.logical(obs_Xd[,2]),each=100), alpha=..count..)) + #,fill = rep(as.logical(obs_Xd[,2]), each=100)
  ggfx::with_outer_glow(colour = '#ffffff', sigma=1, expand=3, x = geom_smooth(aes(y=clin_symptoms2$pred[1:100,], x=cs_miss[1:100,], color = rep(as.logical(obs_Xd[,2]),each=100),fill = rep(as.logical(obs_Xd[,2]), each=100)), alpha=.5,method='loess', se=FALSE)) + 
...
cs_plot$plot2 =
  ggplot()+
  geom_density(aes(x=resid_missingess(motor_palsy2$pred[1:500,], cs_miss[1:500,]), fill=rep(as.logical(obs_Xd[,2]),each=500)), color='transparent', alpha=.5, adjust=1) +
  scale_discrete_manual(c('color', 'fill'), name = '', breaks = c(FALSE, TRUE), labels=c('Imputed', 'Observed'), values = unlist(color) |> setNames(c('FALSE', 'TRUE')))+
  xlab('Residual') +
  ylab('Density') +
  theme_bw()
...
cs_plot$plot3 =
  ggplot()+
  geom_hex((aes(y=resid_missingess(clin_symptoms2$pred[1:100,], cs_miss[1:100,]), x = cs_miss[1:100,], fill=rep(as.logical(obs_Xd[,2]),each=100), alpha=..count..))) +
  ggfx::with_outer_glow(colour = '#ffffff', sigma=1, expand=3, x = geom_smooth((aes(y=resid_missingess(clin_symptoms2$pred[1:100,], cs_miss[1:100,]), x = cs_miss[1:100,], color=rep(as.logical(obs_Xd[,2]),each=100))), method='loess',se=FALSE)) +
  scale_x_continuous(trans='logit') +
  scale_discrete_manual(c('color', 'fill'), name = '', breaks = c(FALSE, TRUE), labels=c('Imputed', 'Observed'), values = unlist(color) |> setNames(c('FALSE', 'TRUE')))+
  scale_alpha_continuous(range=c(0.2,.7), trans='log10') + guides(alpha='none', fill='none')+
  xlab('Pr (Response)') +
...
mp_plot = list()
mp_pred_smooth = motor_palsy2$pred
mp_pred_smooth[,obs_Xd[,3]==1] = sapply(seq_len(dim(mp_miss)[1]), 
                        function(i) loess(Xd[obs_Xd[,3]==1, 3]~mp_miss[i, obs_Xd[,3]==1]) |> predict()) |> t()



mp_plot$plot1 = 
  ggplot() + 
  geom_hex(aes(y=motor_palsy2$pred[1:100,], x=mp_miss[1:100,], fill = rep(as.logical(obs_Xd[,3]),each=100), alpha=..count..)) + #,fill = rep(as.logical(obs_Xd[,3]), each=100)
  ggfx::with_outer_glow(colour = '#ffffff', sigma=1, expand=3, x = geom_smooth(aes(y=motor_palsy2$pred[1:100,], x=mp_miss[1:100,], color = rep(as.logical(obs_Xd[,3]),each=100),fill = rep(as.logical(obs_Xd[,3]), each=100)), alpha=.5,method='loess',span=1, se=FALSE)) + 
...
mp_plot$plot2 = 
  ggplot() +
  geom_density(aes(x=resid_missingess(motor_palsy2$pred, mp_miss)[1:500,], fill=rep(as.logical(obs_Xd[,3]),each=500)), color='transparent', alpha=.5, adjust=.5) +
  scale_discrete_manual(c('color', 'fill'), name = '', breaks = c(FALSE, TRUE), labels=c('Imputed', 'Observed'), values = unlist(color) |> setNames(c('FALSE', 'TRUE')))+
  xlab('Residual') +
  ylab('Density') +
  #xlim(-1, 1)+
...
mp_plot$plot3 = 
  ggplot()+
  geom_hex((aes(y=resid_missingess(motor_palsy2$pred, mp_miss)[1:100,], x = mp_miss[1:100,], fill=rep(as.logical(obs_Xd[,3]),each=100), alpha=..count..))) +
  ggfx::with_outer_glow(colour = '#ffffff', sigma=1, expand=3, x = geom_smooth((aes(y=resid_missingess(motor_palsy2$pred, mp_miss)[1:100,], x = mp_miss[1:100,], color=rep(as.logical(obs_Xd[,3]),each=100))),method='loess', se=FALSE)) +
  scale_x_continuous(trans='logit', breaks = c(.5, .9, .99, .999, .9999)) +
  scale_discrete_manual(c('color', 'fill'), name = '', breaks = c(FALSE, TRUE), labels=c('Imputed', 'Observed'), values = unlist(color) |> setNames(c('FALSE', 'TRUE')))+
  scale_alpha_continuous(range=c(0.2,.7), trans='log10') + guides(alpha='none', fill='none')+
  xlab('Pr (Response)') +
...
id_plot$plot1 = 
  ...
  scale_discrete_manual(c('color', 'fill'), name = '', breaks = c(FALSE, TRUE), labels=c('Imputed', 'Observed'), values = unlist(color) |> setNames(c('FALSE', 'TRUE')))+
  scale_alpha_continuous(range=c(0.2,.7), trans='log10') + guides(alpha='none', fill='none')+
  ylab("log(Symptom Duration (days))")+
  xlab('Pr (Response)') +
  theme_bw()

id_plot$plot2 = 
  ggplot()+
  geom_density(aes(x=resid_missingess(id2$pred, id_miss)[1:500,], fill=rep(as.logical(obs_Xc[,1]),each=500)), color='transparent', alpha=.5, adjust=2) +
  scale_discrete_manual(c('color', 'fill'), name = '', breaks = c(FALSE, TRUE), labels=c('Imputed', 'Observed'), values = unlist(color) |> setNames(c('FALSE', 'TRUE')))+
  xlab('Residual') +
  ylab('Density') +
  theme_bw()
...
id_plot$plot3 = 
  ggplot()+
  geom_hex((aes(y=resid_missingess(id2$pred, id_miss)[1:100,], x = id_miss[1:100,], fill=rep(as.logical(obs_Xc[,1]),each=100), alpha=..count..))) + 
...
gcs_plot$plot1 = 
  ggplot() + 
  geom_hex(aes(y=gcs2$pred2[1:100,] + rnorm(length(gcs2$pred2[1:100,]),0,.05), x=gcs_miss[1:100,], fill = rep(as.logical(obs_Xc[,8]),each=100), alpha=..count..)) + #,fill = rep(as.logical(obs_Xc[,8]), each=100)
  ggfx::with_outer_glow(colour = '#ffffff', sigma=1, expand=3, x = geom_smooth(aes(y=gcs2$pred2[1:100,], x=gcs_miss[1:100,], color = rep(as.logical(obs_Xc[,8]),each=100),fill = rep(as.logical(obs_Xc[,8]), each=100)), alpha=.5,method='loess', span=1, se=FALSE)) + 
...
gcs_plot$plot2 = 
  ggplot()+
  geom_density(aes(x=resid_missingess(gcs2$pred2, gcs_miss)[1:1000,], fill=rep(as.logical(obs_Xc[,8]),each=1000)), color='transparent', alpha=.5, adjust=4) +
  scale_discrete_manual(c('color', 'fill'), name = '', breaks = c(FALSE, TRUE), labels=c('Imputed', 'Observed'), values = unlist(color) |> setNames(c('FALSE', 'TRUE')))+
  xlab('Residual') +
  ylab('Density') +
  theme_bw()
...
gcs_plot$plot3 = 
  ggplot()+
  geom_jitter((aes(y=resid_missingess(gcs2$pred2, gcs_miss)[1:100,], x = gcs_miss[1:100,], color=rep(as.logical(obs_Xc[,8]),each=100))), size=2, shape=21) +
  ggfx::with_outer_glow(colour = '#ffffff', sigma=1, expand=3, x = geom_smooth((aes(y=resid_missingess(gcs2$pred2, gcs_miss)[1:100,], x = gcs_miss[1:100,], color=rep(as.logical(obs_Xc[,8]),each=100))),method='loess',span=1, se=FALSE)) +
  scale_x_continuous(trans='logit', breaks = c(.4, .8, .95, .99, .999)) +
  scale_discrete_manual(c('color', 'fill'), name = '', breaks = c(FALSE, TRUE), labels=c('Imputed', 'Observed'), values = unlist(color) |> setNames(c('FALSE', 'TRUE')))+
  scale_alpha_continuous(range=c(0.2,.7), trans='log10') + guides(alpha='none', fill='none')+
  xlab('Pr (Response)') +
