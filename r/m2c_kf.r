...
m2ckf_folds <- repeated_kfold(m2ckf_input, K = 10, N_rep = 5, N_obs = nrow(data_19EI), seed = 612)
m2ckf_inputs <- m2ckf_folds$inputs
dir.create("outputs/m2c_kf20", showWarnings = F)
ss_sampler <- rstan::stan_model("stan/m2c_kf.stan")
ss <- stan_kfold(sampler = ss_sampler,
                 list_of_datas=m2ckf_inputs,
                 backend = "rstan",
                 chains=3, cores=19,
                 control = list(adapt_delta=.76, max_treedepth=12), init_r = 1, seed=290693,
...
kk <- kfold(ee)
p <- extract_K_fold(ss, holdout, pars=c("theta", "p_Smear", "p_Mgit", "p_Xpert"))
...
ss2=rstan::sflist2stanfit(ss)
p2 = sapply(c("p_Smear", "p_Mgit", "p_Xpert"), 
            function(x) rstan::summary(ss2,x)$summary, simplify = F)

ggplot(mapping=aes(x=p2$p_Smear[,"mean"], y=as.numeric(m2ckf_input$Y_Smear_all))) + 
  stat_smooth(method="glm", formula=y~splines::ns(x,5), se=F) + 
  stat_smooth(mapping=aes(x=p_summary$p_Smear$mean), 
              method="glm", formula=y~splines::ns(x,5), color="red", fill="red", se=F) + 
  geom_line(aes(y=p2$p_Smear[,"mean"])) +
  # ylim(0,1)+
  ggplot(mapping=aes(x=p2$p_Mgit[,"mean"], y=as.numeric(m2ckf_input$Y_Mgit_all))) + 
  stat_smooth(method="glm", formula=y~splines::ns(x,5), se=F) + 
  stat_smooth(mapping=aes(x=p_summary$p_Mgit$mean,), 
             method="glm", formula=y~splines::ns(x,5), color="red", fill="red", se=F) +
  geom_line(aes(y=p2$p_Mgit[,"mean"])) + 
  # ylim(0,1)+
  ggplot(mapping=aes(x=p2$p_Xpert[,"mean"], y=as.numeric(m2ckf_input$Y_Xpert_all))) + 
  stat_smooth(method="glm", formula=y~splines::ns(x,5),se=F) + 
  stat_smooth(mapping=aes(x=p_summary$p_Xpert$mean), 
             method="glm", formula=y~splines::ns(x,5), color="red", fill="red",se=F) +
  geom_line(aes(y=p2$p_Xpert[,"mean"]))
  # ylim(0,1)
...
ggplot(mapping=aes(x=p2$p_Smear[,"mean"], y=as.numeric(m2ckf_input$Y_Smear_all))) + 
  ...
  stat_smooth(mapping=aes(x=p_summary$p_Smear$mean), 
              method="loess", formula=y~x, color="red", fill="red") + 
  geom_line(aes(y=p2$p_Smear[,"mean"])) +
  # ylim(0,1)+
  ggplot(mapping=aes(x=p2$p_Mgit[,"mean"], y=as.numeric(m2ckf_input$Y_Mgit_all))) + 
  stat_smooth(method="loess", formula=y~x) + 
...
p2 <- extract_K_fold(ss, holdout, pars=c("theta", "p_Smear", "p_Mgit", "p_Xpert"), holdout=0)
p2_summary <- sapply(p2, apply, 2, function(l) data.frame(mean = mean(l), median = median(l), CI2.5 = quantile(l, .25), CI97.5 = quantile(l, .975)), simplify = FALSE, USE.NAMES = TRUE)
