...
Xc <- data_19EI %$% cbind(
  (age-mean(age, na.rm=TRUE))/10,       #1    #8
  log2(clin_illness_day),               #2    #9
  sqrt(BLDGLU),                         #3    #10 
  sqrt(csf_glucose),                    #4    #11
  log2(csf_lympho+1),                   #5    #12
  log2(csf_protein),                    #6    #13
  log2(csf_lactate)                     #7    #14
)
...
res_2i_extI_dGCS   <- model_2i_extI_dGCS$sample(data = model_input, seed = 29061993, output_dir = 'outputs',
...
bayesplot::mcmc_intervals(draw_2i_extI_dGCS, pars=vars(dplyr::starts_with("a")))
...
res_2i_extI_dGCS.2   <- rstan::sampling(model_2i_extI_dGCS, chain=3, cores=3,
                                        data = model_input, iter = 3000, warmup = 1000,
                                        include = TRUE,
