...
transformed parameters {
  vector[nX] a_raw; //Add HIV coef to the vector of coef
  vector[nX] a;
  real b_HIV;
  real b_cs;
  vector[nB] b;
...
#include includes/main_prior/m0.stan
          //TODO fix this upstream include includes/main_prior/m.stan
      ...
      if (obs_Xd[n,1] == 1){
        ...
        for (i in 1:N_pattern){
          ...
          if (obs_Xd[n,2] == 1){
            ...
            vector[4] z_Smear_RE = z_Smear[2] + b_FE[1]*bac_load + b_RE[1]*(RE[n] + square(RE[n])*quad_RE);
            vector[4] z_Mgit_RE  = z_Mgit[2]  + b_FE[2]*bac_load + b_RE[2]*(RE[n] + square(RE[n])*quad_RE);
            vector[4] z_Xpert_RE = z_Xpert[2] + b_FE[3]*bac_load + b_RE[3]*(RE[n] + square(RE[n])*quad_RE);
...
#include includes/impute_model/generate_X_CV.stan
    ...
    theta = inv_logit(a0 + X*a);
    {
      ...
      bac_load = b_HIV*X[:,1] + b_cs*X[:,2] + X[:,B2]*b;
      // for (n in 1:N_all) bac_load += RE_all[n] +square(RE_all[n])*quad_RE;
