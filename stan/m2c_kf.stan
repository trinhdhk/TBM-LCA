...
model {
  int nu = 5;
  // Imputation model ---------------------------------------------------------
#include /includes/impute_model/main_part.stan 
...
#include /includes/impute_model/loop_part.stan
...
#include /includes/impute_model/generate_X_CV.stan
    ...
    
    {
      ...
      p_Smear = (1 - theta) * inv_logit(z_Smear[1]) + theta .* inv_logit(z_Smear[2] + b_RE[1]*bac_load);
      p_Mgit  = (1 - theta) * inv_logit(z_Mgit[1])  + theta .* inv_logit(z_Mgit[2]  + b_RE[2]*bac_load);
