...
data {
  ...
  // control for PCA
  int<lower=1, upper=5> K_pca;
}

transformed data {
  
  // vector[K_pca] O = rep_vector(0., K_pca);
  // matrix[K_pca, K_pca] I = diag_matrix(rep_vector(1., K_pca));
...
parameters{
  ...
  // Latent PCA for lab test
  ordered[K_pca] csf_pca[N];
  matrix<lower=0, upper=1>[5,K_pca] csf_W;
  real<lower=0> csf_s;
  ...
  vector[nX + 1 /*gluratio*/ - 1 /*hiv*/ - 6+K_pca /*pca*/] a_; // Extra 1 is for sqrt(glu_ratio) = sqrt(csf_glu)/sqrt(bld_glu) //-3 is reduction after PCA
...
transformed parameters {
  vector[nX + 1 - 6+K_pca] a = append_row(a_pos, a_); //Add HIV coef to the vector of coef
...
model {
  ...
  vector[nXc + 1 - 6+K_pca] Xc_imp[N]; //fully imputed cont X
  matrix[N, nX + 1 - 3 - 6+K_pca] X_compl;
  real GCSV_imp[N]; 
  vector[5] csf_imp[N];
  int nu = 5;
  ...
  //CSF lab
  {
    vector[5] csf_imp_centered[N];
    csf_imp[:,1:5] = impute_cont_2d(Xc[:,3:7], obs_Xc[:,3:7], append_array(append_array(bld_glu_imp, csf_glu_imp), csf_other_imp)); //CSF Lab tests
    // csf_imp[:,6]  = to_array_1d(to_vector(csf_imp[:,2]) ./ to_vector(csf_imp[:,1])); //Glucose ratio
    csf_imp_centered = center(csf_imp);
    
    to_vector(csf_W) ~ normal(0, 1);
    csf_s ~ normal(0, 1);
    for (n in 1:N){
      // csf_pca[n] ~ multi_normal(O, I);
      csf_pca[n] ~ normal(0, 1);
      csf_imp_centered[n] ~ normal(csf_W * csf_pca[n], csf_s);
    }
    Xc_imp[:,3:3+K_pca-1] = csf_pca;
  }
  
  // - GCSV
  GCSV_imp = impute_cont_1d(Tc[:,3], obs_Tc[:,3], gcsv_imp); //GCSV
  Xc_imp[:,3+K_pca] = to_array_1d(log2(to_vector(Tc[:,1]) + to_vector(Tc[:,2]) + to_vector(GCSV_imp) + 1));

 
  if (nXc > 8) // Other if exists
    for (j in 9:nXc) Xc_imp[:, j + 1- 6+K_pca] = Xc[:, j];
  ...
  
  {
    ...
    csf_imp ~ multi_normal_cholesky(csf_mu, L_Sigma_csf);
...
#include /includes/main_priors.stan
  ...
  for (n in 1:N){
    ...
    if (N_Xd_miss > 0){ //if there is some discrete variables missing
      ...
      pat_thetas[2] += a0 + dot_product(a[4:num_elements(a)], X_compl[n]);
      ...
      // The normal way
      row_vector[nX + 1 - 6+K_pca] X = append_col(Xd_imp[n,:], X_compl[n,:]);
...
generated quantities{
  ...
  matrix[N_all, nX + 1 - 6+K_pca] X;
  {
    ...
    
    {
      ...
      vector[nXc + 1 - 6+K_pca] Xc_imp[N_all]; //fully imputed cont X
      ...
      
      {
        ...
        vector[5] csf_all[N_all];
        vector[5] csf_err[N_all];
        vector[5] csf_mu[N_all];
        
        vector[K_pca] csf_pca_all[N_all];
        int j = 1;
        for (n in which_not(keptin)){
        ...
        csf_all[which_not(keptin), 1:5] = multi_normal_cholesky_partial_rng(Xc_all[which_not(keptin),3:7], obs_Xc_all[which_not(keptin),3:7], csf_mu_valid, L_Omega_csf);
        csf_all[which(keptin), 1:5]     = impute_cont_2d(Xc[:,3:7], obs_Xc[:,3:7], append_array(append_array(bld_glu_imp, csf_glu_imp), csf_other_imp));
        // csf_all[:, 6] = to_array_1d(to_vector(csf_all[:,2]) ./ to_vector(csf_all[:,1]));
        csf_all = center(csf_all);
        
        for (n in 1:N_all){
          csf_err[n] = to_vector(normal_rng(rep_vector(0,5 /*6*/), csf_s)); 
          /*because csf ~ N(mu, csf_s) hence csf-mu = csf_err ~ N(0, csf_s); We random this substraction and then subtract it from csf to get the mu*/
          csf_mu[n] = csf_all[n] - csf_err[n];
          csf_pca_all[n] = pseudo_inverse(csf_W) * (csf_mu[n]);
        }
        
        Xc_imp[:, 3:3+K_pca-1] = csf_pca_all;
      }
      
      {
        ...
        Xc_imp[:,3+K_pca] = to_array_1d(log2(to_vector(Tc_all[:,1]) + to_vector(Tc_all[:,2]) + to_vector(GCSV_imp_all) + 1));
      }
      
      if (nXc > 8) // Other if exists
        for (j in 9:nXc) Xc_imp[:, j + 1 - 6+K_pca] = Xc_all[:, j];
    ...
    
    {
      ...
      bac_load = b_HIV*X[:,1] + b_RE*RE_all;
