...
#include includes/main_prior/penalty.stan
  ...
  if (penalty_family == 0) {
    a2_raw ~ student_t(nu, 0, 2);
  }
  
  if (penalty_family == 1) {
    a2_raw ~ double_exponential(0, 2);
  }
    ...
    if (N_Xd_miss > 0){ //if there is some discrete variables missing
      ...
      if (obs_Xd[n,1] == 1){
        for (i in 1:N_pattern){
          ...
          real theta = inv_logit(pat_thetas[2,i] + a2*Xd_imp[n,1]*X_compl[n,5]);
        ...
        for (i in 1:N_pattern){
          ...
          real bac_load;
          if (i % 2 == 0){
            theta = inv_logit(pat_thetas[2,i] + a2*X_compl[n,5]);
      ...
      real theta = inv_logit(a0 + dot_product(a, X) + a2*X[1]*X[8]);
