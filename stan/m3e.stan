...
data {
  ...
  int<lower=0, upper=1> Y_NegCrypto_all[N_all];
...
#include includes/impute_model/transform_data.stan
  int<lower=0, upper=1> Y_NegCrypto[N] = Y_NegCrypto_all[which(keptin)];
...
#include includes/main_prior/penalty.stan
  ...
  z_NegCrypto[1] ~ logistic(logit(.97), .5);
  z_NegCrypto[2] ~ logistic(0, .37);
      ...
      if (obs_Xd[n,1] == 1){
        for (i in 1:N_pattern){
          ...
          log_liks[i] = logprob_theta + log_mix(theta, 
            bernoulli_logit_lpmf(Y_NegCrypto[n] | z_NegCrypto[2]) + bernoulli_logit_lpmf(Y_Xpert[n] | z_Xpert_RE) + bernoulli_logit_lpmf(Y_Mgit[n] | z_Mgit_RE) + bernoulli_logit_lpmf(Y_Smear[n] | z_Smear_RE),
            bernoulli_logit_lpmf(Y_NegCrypto[n] | z_NegCrypto[1]) + bernoulli_logit_lpmf(Y_Xpert[n] | z_Xpert[1]) + bernoulli_logit_lpmf(Y_Mgit[n] | z_Mgit[1]) + bernoulli_logit_lpmf(Y_Smear[n] | z_Smear[1]));
        }
      } else {
        for (i in 1:N_pattern){
          real logprob_theta = pat_thetas[1,i];
...
#include includes/impute_model/generate_X_CV.stan
    ...
    
    {
      ...
      p_NegCrypto = (1 - theta) * inv_logit(z_NegCrypto[1]) + theta * inv_logit(z_NegCrypto[2]);
      
#include includes/generated_quantities/pairwise_corr.stan
      
      for (n in 1:N_all){
        log_lik[n] = log_mix(theta[n],
        bernoulli_logit_lpmf(Y_NegCrypto_all[n] | z_NegCrypto[2]) + bernoulli_logit_lpmf(Y_Xpert_all[n] | z_Xpert_RE[n]) + bernoulli_logit_lpmf(Y_Mgit_all[n] | z_Mgit_RE[n]) + bernoulli_logit_lpmf(Y_Smear_all[n] | z_Smear_RE[n]),
        bernoulli_logit_lpmf(Y_NegCrypto_all[n] | z_NegCrypto[1]) + bernoulli_logit_lpmf(Y_Xpert_all[n] | z_Xpert[1]) + bernoulli_logit_lpmf(Y_Mgit_all[n] | z_Mgit[1]) + bernoulli_logit_lpmf(Y_Smear_all[n] | z_Smear[1])
        );
      }
    }
  }
