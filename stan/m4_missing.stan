    ...
    if (N_Xd_miss > 0){ //if there is some discrete variables missing
      ...
      if (obs_Xd[n,1] == 1){
        for (i in 1:N_pattern){
          ...
          real z_Smear_RE = z_Smear[2] + b_FE[1]*bac_load + b_RE[1]*(RE[2,n] + square(RE[n])*quad_RE);
          real z_Mgit_RE  = z_Mgit[2]  + b_FE[2]*bac_load + b_RE[2]*(RE[2,n] + square(RE[n])*quad_RE);
          real z_Xpert_RE = z_Xpert[2] + b_FE[3]*bac_load + b_RE[3]*(RE[2,n] + square(RE[n])*quad_RE);
...
#include includes/impute_model/generate_X_CV.stan
    ...
    theta = inv_logit(a0 + X*a);
    {
      ...
      for (i in 1:2){
        RE_all[which(keptin),i] = RE[:,i];
        for (n in which_not(keptin)) RE_all[n,i] = normal_rng(0,1);
      }
