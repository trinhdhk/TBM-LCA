functions {
  ...
  // This function do modulo for real
  real real_modulo(real x, int n){
    real resid = x;
    while ((resid - n) > 0) {
      resid -= n;
    }
...
model {
  ...
  
  {
    ...
    for (n in 1:N){
      ...
      // if (real_modulo(GCSV_imp[n],1) > 0) target += negative_infinity();
    }
...
generated quantities{
  ...
  matrix[N, nXc + nXd] X = append_col(append_col(Xd_rng, Xd_compl), Xc_imp);
