...
model {
  ...
  p_Img = Phi(z_Img);
  p_LymGluRatio = Phi(z_LymGluRatio);
  
  for (n in 1:N){
    real x = 0;
    target += log_mix(theta[n],
    bernoulli_lpmf(Y_Xpert[n] | p_Xpert[n,2]) + bernoulli_lpmf(Y_Mgit[n] | p_Mgit[n,2]) + bernoulli_lpmf(Y_Smear[n] | p_Smear[n,2]) + bernoulli_lpmf(Y_Img[n] | p_Img[2]) + bernoulli_lpmf(Y_LymGluRatio[n] | p_LymGluRatio[2]),
    bernoulli_lpmf(Y_Xpert[n] | p_Xpert[n,1]) + bernoulli_lpmf(Y_Mgit[n] | p_Mgit[n,1]) + bernoulli_lpmf(Y_Smear[n] | p_Smear[n,1]) + bernoulli_lpmf(Y_Img[n] | p_Img[1]) + bernoulli_lpmf(Y_LymGluRatio[n] | p_LymGluRatio[1]));
  }
}
