functions {
  ...
  vector impute_discrete_1d(int[] raw, int[] obs, real[] z){
    int N = size(raw);
    vector[N] imp;
...
transformed parameters{
  ...
  // - HIV
  Xd_imp[,1] = impute_discrete_1d(Xd[:,1], obs_Xd[:,1], rep_array(Phi(HIV_a0), N));
  ...
  Xd_imp[,2] = impute_discrete_2d(Xd[,2], obs_Xd[,2], to_array_2d(append_all(z_cs)), obs_cs);
